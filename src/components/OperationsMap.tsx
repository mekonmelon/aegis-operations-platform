...
import type { Coordinates, Facility, Incident } from '../types/domain'

const facilityIcons = { hospital: Cross, shelter: TentTree, depot: Package }

type MappedIncident = Incident & { coordinates: Coordinates }
...
export function OperationsMap({ incidents, facilities, selectedIncidentId, onSelectIncident }: OperationsMapProps) {
  const mappedIncidents = incidents.filter((incident): incident is MappedIncident => Boolean(incident.coordinates))
